...
/**
 * An assets bundle (like {@link io.alakazam.assets.AssetsBundle}) that utilizes configuration to provide the
...
public class ConfiguredAssetsBundle implements ConfiguredBundle<AssetsBundleConfiguration> {
     ...
     * @param uriPath          the uri path for the static asset files
     */
    public ConfiguredAssetsBundle(String resourcePath, CacheBuilderSpec cacheBuilderSpec, String uriPath, String indexFile) {
    ...
    public void run(AssetsBundleConfiguration bundleConfig, Environment env) throws Exception {
        ...
        boolean staticAssets = config.isStaticAssets();

        AssetServlet servlet = new AssetServlet(resourcePath, spec, uriPath, indexFile, overrides, mimeTypes);
        servlet.setStaticAssets(staticAssets);
        String servletId = uriPath.replace("/", "");
        env.servlets().addServlet(servletId, servlet).addMapping(uriPath + "*");
    }
