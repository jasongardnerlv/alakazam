...
import io.alakazam.servlets.assets.ResourceURL;
 ...
 * {@link io.alakazam.servlets.assets.AssetServlet} with the exception of allowing for override options.
 *
 * @see io.alakazam.servlets.assets.AssetServlet
 */
class AssetServlet extends HttpServlet {
    ...
    public AssetServlet(String resourcePath, CacheBuilderSpec spec, String uriPath, String indexFile,
        ...
        loader = new AssetLoader(resourcePath, uriPath, indexFile, overrides);
    ...
    // JG 12/10/2014 - config value used to force static only assets
    public void setStaticAssets(boolean isStatic) {
        loader.setStaticAssets(isStatic);
    }
    ...
    private static class AssetLoader extends CacheLoader<String, Asset> {
        ...
        private boolean staticAssets; // JG 12/10/2014 - config value used to force static only assets
        ...
        private Asset loadOverride(String key) throws Exception {
            ...
            for (Map.Entry<String, String> override : overrides) {
                ...
                if (file.exists()) {
                    // JG 12/10/2014 - config value used to force static only assets
                    if (staticAssets) {
                        return new StaticAsset(Files.toByteArray(file), file.lastModified());
                    } else {
